<?php
/**
 * Smart Customizer functionality
 *
 * @package WordPress
 * @subpackage Smart
 * @since SMART 1.0
...
function Smart_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'Smart_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'Smart' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => Smart_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'Smart' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'Smart' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'Smart' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'Smart' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'Smart' );
}
add_action( 'customize_register', 'Smart_customize_register', 11 );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Smart 1.5
 * @see Smart_customize_register()
 *
 * @return void
 */
...
/**
 * Register color schemes for Smart.
 *
 * Can be filtered with {@see 'Smart_color_schemes'}.
 *
 ...
 */
function Smart_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Smart.
	 *
	 ...
	 * }
	 */
	return apply_filters( 'Smart_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'Smart' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'Smart_get_color_scheme' ) ) :
/**
 * Get the current Smart color scheme.
 *
 * @since SMART 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function Smart_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = Smart_get_color_schemes();
...
endif; // Smart_get_color_scheme

if ( ! function_exists( 'Smart_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for Smart.
 *
 * @since SMART 1.0
 *
 * @return array Array of color schemes.
 */
function Smart_get_color_scheme_choices() {
	$color_schemes                = Smart_get_color_schemes();
...
endif; // Smart_get_color_scheme_choices

if ( ! function_exists( 'Smart_sanitize_color_scheme' ) ) :
/**
 ...
 */
function Smart_sanitize_color_scheme( $value ) {
	$color_schemes = Smart_get_color_scheme_choices();
...
endif; // Smart_sanitize_color_scheme
 ...
 */
function Smart_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = Smart_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = Smart_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = Smart_get_color_scheme_css( $colors );

	wp_add_inline_style( 'Smart-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'Smart_color_scheme_css' );
 ...
 * @since SMART 1.0
 */
function Smart_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', Smart_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'Smart_customize_control_js' );
...
function Smart_customize_preview_js() {
	wp_enqueue_script( 'Smart-customize-preview', get_template_directory_uri() . '/assets/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
 ...
 */
function Smart_get_color_scheme_css( $colors ) {
 ...
 * @since SMART 1.0
 */
function Smart_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-Smart-color-scheme">
		<?php echo Smart_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'Smart_color_scheme_css_template' );
